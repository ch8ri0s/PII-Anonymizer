...
export class FeedbackAggregator {
     ...
     */
    aggregatePatterns(events, entityField) {
     ...
     */
    createKeyString(pattern, entityType, documentType) {
     ...
     */
    static fromLegacyEntries(entries, source = 'desktop') {
        const events = entries.map(entry => {
            // Map legacy action to feedback action
            const action = entry.action === 'DISMISS'
                ? 'mark_as_not_pii'
                : entry.action === 'ADD'
                    ? 'mark_as_pii'
                    : 'mark_as_not_pii';
 ...
 */
export function createAggregator(events, options) {
