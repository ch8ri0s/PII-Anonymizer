...
// Make selectedFiles accessible for integration
...
// Markdown preview buttons
copyMarkdownBtn.addEventListener('click', copyMarkdownToClipboard);
closePreviewBtn.addEventListener('click', closeMarkdownPreview);
...
function addFile(fileObj) {
  ...
  if (['.doc','.docx','.xls','.xlsx','.csv','.pdf','.txt'].includes(ext)) {
    if (!selectedFiles.find((f) => f.path === fileObj.path)) {
      ...
      // Trigger file preview if available
      if (window.addFileToPreview && fileObj.path) {
        window.addFileToPreview(fileObj.path);
      }
    }
  } else {
...
function copyMarkdownToClipboard() {
  if (!currentMarkdown) {
    showStatus('No markdown content to copy', 'error');
    return;
  }
